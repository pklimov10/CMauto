...
bb7=`$my_java_home -cp $JDBCFILELOCATION":/u01/CM/script/SoLoader/"  PostgresqlQueryExecuteJDBC  -h $IP_AMGR -p $PORT_AMGR -U $DB_AMGR_USER -W $DB_AMGR_PASS -d $DB_AMGR_NAME -c "WITH mass AS ( 	SELECT 		1 state 	FROM 		am_trigger_executions amm 	JOIN 		am_agent_trigger trigger 			ON trigger.id = amm.owner 	WHERE 		trigger.title = 'CMJ-HRGateway-SignalsProcessingAgent' AND 	amm.execution_status = 'STOPPED' AND 		amm.created_date IN ( 			SELECT 				max(amm.created_date) 			FROM 				am_trigger_executions amm 			JOIN 				am_agent_trigger trigger 					ON trigger.id = amm.owner 			WHERE 				trigger.title = 'CMJ-HRGateway-SignalsProcessingAgent' 		) UNION 	SELECT 		1 state 	FROM 		am_trigger_executions amm 	JOIN 		am_agent_trigger trigger 			ON trigger.id = amm.owner 	WHERE 		trigger.title = 'CMJ-HRGateway-SignalsProcessingAgent' AND 		amm.execution_status = 'ERROR' AND 		amm.created_date IN ( 			SELECT 				max(amm.created_date) 			FROM 				am_trigger_executions amm 			JOIN 				am_agent_trigger trigger 				ON trigger.id = amm.owner 			WHERE 				trigger.title = 'CMJ-HRGateway-SignalsProcessingAgent' 		) UNION 	SELECT 		0 state 	FROM 		am_trigger_executions amm 	JOIN 		am_agent_trigger trigger 			ON trigger.id = amm.owner 	WHERE 		trigger.title = 'CMJ-HRGateway-SignalsProcessingAgent' AND 		amm.execution_status = 'RUNNING' AND 		amm.created_date IN ( 			SELECT 				max(amm.created_date) 			FROM 				am_trigger_executions amm 			JOIN 				am_agent_trigger trigger 					ON trigger.id = amm.owner 			WHERE 	trigger.title = 'CMJ-HRGateway-SignalsProcessingAgent' 		) UNION 	SELECT 		0 state 	FROM 		am_trigger_executions amm 	JOIN 		am_agent_trigger trigger 			ON trigger.id = amm.owner 	WHERE 		trigger.title = 'CMJ-HRGateway-SignalsProcessingAgent' AND 		amm.execution_status = 'FINISHED' AND 		amm.created_date IN ( 			SELECT 				max(amm.created_date) 			FROM 				am_trigger_executions amm 			JOIN 				am_agent_trigger trigger 					ON trigger.id = amm.owner 			WHERE 				trigger.title = 'CMJ-HRGateway-SignalsProcessingAgent' ) UNION 	SELECT 		1 state 	FROM 		am_trigger_executions amm 	JOIN 		am_agent_trigger trigger 			ON trigger.id = amm.owner 	WHERE 		trigger.title = 'CMJ-HRGateway-SignalsProcessingAgent' AND 		amm.execution_status = 'WAITING' AND 		amm.created_date < current_timestamp - INTERVAL '20' MINUTE AND 		amm.created_date IN ( 			SELECT 				max(amm.created_date) 			FROM 				am_trigger_executions amm 			JOIN 				am_agent_trigger trigger 					ON trigger.id = amm.owner 			WHERE 				trigger.title = 'CMJ-HRGateway-SignalsProcessingAgent' 		) UNION 	SELECT 		0 state 	FROM 		am_trigger_executions amm 	JOIN 		am_agent_trigger trigger 			ON trigger.id = amm.owner 	WHERE 		trigger.title = 'CMJ-HRGateway-SignalsProcessingAgent' AND 		amm.execution_status = 'WAITING' AND 		amm.created_date > current_timestamp - INTERVAL '20' MINUTE AND 		amm.created_date IN ( 			SELECT 				max(amm.created_date) 			FROM 	am_trigger_executions amm 			JOIN 				am_agent_trigger trigger 					ON trigger.id = amm.owner 			WHERE 				trigger.title = 'CMJ-HRGateway-SignalsProcessingAgent' 		) 		UNION 	SELECT 		1 state FROM 	am_trigger_executions amm JOIN 	am_agent_trigger trigger 		ON trigger.id = amm.owner JOIN 	am_trigger_agent_exec exec 		ON exec.owner = amm.id JOIN 	am_agent_exec_log log 	ON log.owner = exec.id WHERE 	trigger.title = 'CMJ-HRGateway-SignalsProcessingAgent' AND 	execution_log LIKE '%java%' AND 	log.created_date IN ( 		SELECT 			max(log.created_date) 		FROM 			am_trigger_executions amm 		JOIN 			am_agent_trigger trigger 				ON trigger.id = amm.owner 		JOIN 			am_trigger_agent_exec exec 				ON exec.owner = amm.id 		JOIN 			am_agent_exec_log log 				ON log.owner = exec.id 		WHERE 			trigger.title = 'CMJ-HRGateway-SignalsProcessingAgent' 	) ) TABLE mass ORDER BY state DESC LIMIT 1;" |grep -v state`
if [ "$bb7" -ne "$vv6" ]
then

echo Вывод  агентов CMJ-HRGateway-SignalsProcessingAgent НЕ ОТРАБОТАЛ
else
  echo '#######################################################'
  echo Агент  CMJ-HRGateway-SignalsProcessingAgent  успешно отработал
fi

bb8=`$my_java_home -cp $JDBCFILELOCATION":/u01/CM/script/SoLoader/"  PostgresqlQueryExecuteJDBC  -h $IP_AMGR -p $PORT_AMGR -U $DB_AMGR_USER -W $DB_AMGR_PASS -d $DB_AMGR_NAME -c "WITH mass AS ( 	SELECT 		1 state 	FROM 		am_trigger_executions amm 	JOIN 		am_agent_trigger trigger 			ON trigger.id = amm.owner 	WHERE 		trigger.title = 'PGTW-StructPublisherAgent' AND 		amm.execution_status = 'STOPPED' AND 		amm.created_date IN ( 			SELECT 				max(amm.created_date) 			FROM 				am_trigger_executions amm 			JOIN 				am_agent_trigger trigger 					ON trigger.id = amm.owner 			WHERE 				trigger.title = 'PGTW-StructPublisherAgent' 		) UNION 	SELECT 		1 state 	FROM 		am_trigger_executions amm 	JOIN 		am_agent_trigger trigger 			ON trigger.id = amm.owner 	WHERE 		trigger.title = 'PGTW-StructPublisherAgent' AND 		amm.execution_status = 'ERROR' AND 		amm.created_date IN ( 	SELECT 				max(amm.created_date) 			FROM 				am_trigger_executions amm 			JOIN 				am_agent_trigger trigger 					ON trigger.id = amm.owner 			WHERE 				trigger.title = 'PGTW-StructPublisherAgent' 		) UNION 	SELECT 		0 state 	FROM 		am_trigger_executions amm 	JOIN 		am_agent_trigger trigger 			ON trigger.id = amm.owner 	WHERE 		trigger.title = 'PGTW-StructPublisherAgent' AND 		amm.execution_status = 'RUNNING' AND 		amm.created_date IN ( 			SELECT 				max(amm.created_date) 			FROM 				am_trigger_executions amm 			JOIN 				am_agent_trigger trigger 					ON trigger.id = amm.owner 			WHERE 				trigger.title = 'PGTW-StructPublisherAgent' 		) UNION 	SELECT 		0 state 	FROM 		am_trigger_executions amm 	JOIN 		am_agent_trigger trigger 			ON trigger.id = amm.owner 	WHERE 		trigger.title = 'PGTW-StructPublisherAgent' AND 		amm.execution_status = 'FINISHED' AND 		amm.created_date IN ( 			SELECT 				max(amm.created_date) 			FROM 				am_trigger_executions amm 		JOIN 				am_agent_trigger trigger 					ON trigger.id = amm.owner 			WHERE 				trigger.title = 'PGTW-StructPublisherAgent' 		) UNION 	SELECT 		1 state FROM 		am_trigger_executions amm 	JOIN 		am_agent_trigger trigger 			ON trigger.id = amm.owner 	WHERE 		trigger.title = 'PGTW-StructPublisherAgent' AND 		amm.execution_status = 'WAITING' AND 		amm.created_date < current_timestamp - INTERVAL '20' MINUTE AND 		amm.created_date IN ( 			SELECT 				max(amm.created_date) 			FROM 				am_trigger_executions amm 			JOIN 	am_agent_trigger trigger 					ON trigger.id = amm.owner 			WHERE 				trigger.title = 'PGTW-StructPublisherAgent' 		) UNION 	SELECT 		0 state 	FROM 		am_trigger_executions amm 	JOIN 		am_agent_trigger trigger 			ON trigger.id = amm.owner 	WHERE 		trigger.title = 'PGTW-StructPublisherAgent' AND 		amm.execution_status = 'WAITING' AND 		amm.created_date > current_timestamp - INTERVAL '20' MINUTE AND 		amm.created_date IN ( 			SELECT 				max(amm.created_date) 			FROM 				am_trigger_executions amm 			JOIN 			am_agent_trigger trigger 					ON trigger.id = amm.owner 			WHERE 				trigger.title = 'PGTW-StructPublisherAgent' 		) ) TABLE mass;" |grep -v state`
if [ "$bb8" -ne "$vv6" ]
then

echo Вывод  агентов PGTW-StructPublisherAgent НЕ ОТРАБОТАЛ
else
  echo '#######################################################'
  echo Агент  PGTW-StructPublisherAgent  успешно отработал
fi
